...
class DateTimeUi {
    ...
    static setRange(pickerObj, key) {
        if (pickerObj.config.inputTo && pickerObj.config.options[key]) {
            ...
            let nextDay = this.getRangeDate(pickerObj.config, key);
            ...
            if (pickerObjTo) {
                if (!pickerObj.hasOwnProperty('DateObj')) {
                    ...
                    this.setValidMinMaxInput(pickerObj, pickerObjTo, dataSetKey, nextDay);
    ...
    static  setValidMinMaxInput (pickerObj, pickerObjTo, dataSetKey, nextDay)
    {
        let minDate = "";
        let maxDate = "";
        let inputVal = "";

        if (dataSetKey === 'minDate') {
            if (pickerObjTo.config?.options?.maxDate) {
                minDate = pickerObjTo.config.value;
                maxDate = this.getRangeDate(pickerObj.config, "maxRange");
                inputVal = nextDay;
            } else {
                minDate = nextDay;
                maxDate = pickerObjTo.config.value;
                inputVal = minDate;
            }
        }

        if (dataSetKey === 'maxDate' && !pickerObjTo.config?.options?.minDate) {
            minDate = pickerObjTo.config.value;
            maxDate = this.getRangeDate(pickerObj.config, "maxRange");
            inputVal = maxDate;
        }

        if (inputVal && !this.checkDateRange(minDate, maxDate)) {
            pickerObjTo.setInput(inputVal);
        }
    }
    ...
    static setMinMaxDate(pickerObj, key) {
        if (pickerObj.config.options[key] !== '') {
            ...
            if (!this.checkDateRange(...checkAgs)) {
                if (key === 'maxDate') {
                    if (pickerObj.updateConfDateMinMax) {
    ...
    static getMinMax(config, key, dateKey, confDate) {
        let result = '';
        if (config.options[key].callerDate.year.toString() === confDate.year.toString()
            && config.options[key].callerDate.month.toString() === confDate.month.toString()) {
            ...
            if (key === "minDateObj") {
                ...
                if (dateKey === "month" && config.options[key].callerDate.year.toString() === confDate.year.toString()) {
                    result = config.options[key].callerDate[dateKey];
                }
            }
        }
    ...
    static getNextDate(args, range) {
        ...
        let next = (args.length === 3)
            ...
            ? new Date(args[0] * 1, args[1], args[2] * 1, 0, 0, range)
    ...
    static getLocaleDateFormat(date) {
...
class TimeObj extends PickerObj
{
    ...
    setInput(value, setListItems = true) {
        ...
        ['minTime', 'maxTime'].map(key => {
            //
            this.setMinMaxTime(key, this.config.options[key]);
        });
        this.setMinutes();
    ...
    setMinMaxTime(key, value) {
        ...
        const min = DateTimeUi.getMinutes(value);
        if (!isNaN(min) && min >= 1440) {
            value = "23:59";
        }
    ...
    setRange(setListItems) {
        if (this.config.inputTo) {
            ...
            keys.map(key => {
                if (this.config.input.dataset?.[key]) {
...
class DateObj extends PickerObj
{
                    ...
                    switch (key) {
                        ...
                        case'maxRange':
                            if (key==="maxDate" || key==="minDate") {
                                this.dataset[key] = DateTimeUi.getLocaleDateFormat(this.dataset[key]);
                            }
    ...
    getValidInput(value) {
        ...
        // When allow empty input value
        if (value.trim() === "" && this.config.input.classList.contains(DateTimeUi.selectorConfig.defaults.emptyValue)) {
            return value;
        }
        value = (this.config.regEx.test(value)) ? value.trim() : this.config.valueCache;
        ...
        if (value) {
            ...
            if (! minDate && maxDate && !DateTimeUi.checkDateRange( value, maxDate)) {
